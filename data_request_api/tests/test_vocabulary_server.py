...
class TestVocabularyServer(unittest.TestCase):
    ...
    def test_get_element(self):
        ...
        with self.assertRaises(ValueError):
            elt = vs.get_element(element_type="my_type",
                                 element_id="test", id_type="type")
        with self.assertRaises(ValueError):
            elt = vs.get_element(
                element_type="variable_comment", element_id="test", id_type="type")

        elt = vs.get_element(element_type="mips",
                             element_id="TIPMIP", id_type="mip_short_name")
        target_dict = {
        ...
        obj = vs.get_element(element_type="mips",
                             element_id="link::default_401")
        self.assertDictEqual(elt, target_dict)

        with self.assertRaises(ValueError):
            obj = vs.get_element(element_type="mips",
                                 element_id="link::default_200")

        obj = vs.get_element(element_type="mips",
                             element_id="link::default_200", default=None)
        self.assertIsNone(obj)

        with self.assertRaises(ValueError):
            obj = vs.get_element(element_type="mips",
                                 element_id=None, id_type="name")

        with self.assertRaises(ValueError):
            obj = vs.get_element(element_type="mips",
                                 element_id="undef", id_type="name")

        obj = vs.get_element(
            element_type="mips", element_id="link::default_401", element_key="mip_long_name")
        self.assertEqual(
            obj, "Tipping Point Modelling Intercomparison Project")

        with self.assertRaises(ValueError):
            obj = vs.get_element(
                element_type="mips", element_id="link::default_401", element_key="long_name")
