...
base = dq.create_dreq_tables_for_request(
    deepcopy(content), consolidated=use_consolidated)
...
for opp_id in opp_ids:
    ...
    for link in opp.variable_groups:
        ...
        assert var_group.name not in opp_vars_by_group[
            opp.title], 'variable group name is not unique in this opportunity!'
        ...
        for link in var_group.variables:
            ...
            if var_name not in opp_vars_by_priority[opp.title][priority_level]:
                ...
                opp_vars_by_priority[opp.title][priority_level].append(
                    var_name)

        if len(opp_vars_by_group[opp.title][var_group.name]) != len(set(opp_vars_by_group[opp.title][var_group.name])):
            raise Exception(
                'overlap between variable groups for opportunity: ' + opp.title)
...
base = dq.create_dreq_tables_for_variables(
    deepcopy(content), consolidated=use_consolidated)
...
var_name_map = {record.compound_name: record_id for record_id,
                record in Vars.records.items()}
assert len(var_name_map) == len(
    Vars.records), 'compound names do not uniquely map to variable record ids'
...
for opp_title in opp_titles:
    ...
    # records a collection of info about a variable, indexed by compound name
    ...
    for priority_level, var_names in opp_vars_by_priority[opp_title].items():
        ...
        for var_name in var_names:
            ...
            # variable record, representing one variable
            ...
            for link in spatial_shape.dimensions:
                ...
                if 'plev' in dims.name:
                    ...
                    if dims.name not in plev_info:
                        ...
                        plev_info[dims.name] = [
                            float(s) for s in dims.requested_values.split()]
    ...
    opp_var_info_by_group[opp_title] = {var_group_name: OrderedDict(
    ) for var_group_name in opp_vars_by_group[opp_title]}
...
if len(opp_vars_at_multiple_priorities) > 0:
    ...
    opp_vars_at_multiple_priorities = {k: sorted(
        v, key=str.lower) for k, v in opp_vars_at_multiple_priorities.items()}
    with open(filepath, 'w') as f:
