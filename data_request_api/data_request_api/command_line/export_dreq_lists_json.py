...
# In-place augmentation after writing the base file
...
def parse_args():
    ...
    # Make output file optional; default to working dir with fixed name
    parser.add_argument(
        'output_file',
        nargs='?',
        default='requested_cmip7names_timesubsetted.json',
        help='file to write JSON output to (default: requested_cmip7names_timesubsetted.json in CWD)'
    )
    ...
    # migrated flags (now live here only)
...
def main():
    ...
    if len(expt_vars['experiment']) > 0:
        ...
        outfile = args.output_file  # defaults to 'requested_cmip7names_timesubsetted.json' in CWD
        ...
        if args.time_subsets or args.add_combined:
            augment_file_in_place(
                base=base,
                outfile=outfile,
                add_combined=args.add_combined,
                quiet=True,
                use_opps=use_opps,   # pass through the user’s chosen opportunities
            )
...
# # NEW: import the in-place augmentation helper from add_timesubsets.py
# from add_timesubsets import augment_file_in_place
...
#     # ---- migrated flags from add_timesubsets.py (CLI only) ----
#     parser.add_argument("-c", "--add_combined", action="store_true",
#                         help="Include combined request from all opportunities and for all experiments "
#                              "(incl. combinations control / historical / scenario).")
#     parser.add_argument("-t", "--time_subsets", action="store_true",
...
#         # >>> NEW: If flags are set, augment the just-written file IN PLACE (reads and overwrites internally)
#         if args.time_subsets or args.add_combined:
#             augment_file_in_place(base=base, outfile=outfile, add_combined=args.add_combined, quiet=True)
