...
# "field" is used often for Airtable column names, so need a different name here
...
# names of priority levels, ordered from highest to lowest priority
    ...
    def __init__(self, record, field_info):
        ...
        for field_name, value in record.items():
            ...
            if 'linked_table_id' in field_info[field_name]:
                assert isinstance(
                    value, list), 'links should be a list of record identifiers'
        ...
        for k, v in self.__dict__.items():
            ...
            if isinstance(v, list):
                ...
                if n > show_list_entries:
                    # s += '\n' + indent + f'... ({n} entries)'
                    s += '\n' + indent + \
                        f'... ({n} in list, first {show_list_entries} shown)'
            else:
...
class dreq_table:
    ...
    def __init__(self, table, table_id2name):
        ...
        # dict giving info on each field, keyed by field_id (example: 'fld61d8b5mzI45H8F')
        fields = table['fields']
        # as fields dict, but use field name as the key
        field_info = {field['name']: field for field in fields.values()}
        ...
        for field_name, field in field_info.items():
            ...
            # remember the Airtable name, in case useful later
            attr2field[attr] = field_name
        ...
        # dict giving info on each record, keyed by record_id (example: 'reczyxsKbAseqCisA')
    ...
    def get_record(self, m):
        if isinstance(m, int):
            ...
            raise TypeError(
                f'Error specifying record to retrieve from table {self.table_name}')
    ...
    def get_record_id(self, record):
        ...
        l = [record_id for record_id, rec in self.records.items() if rec ==
             record]
        if len(l) == 1:
            return l[0]
        else:
...
class expt_request:
    ...
    def __post_init__(self):
        for p in PRIORITY_LEVELS:
            assert hasattr(
                self, p), 'expt_request object missing priority level: ' + p
    ...
    def add_vars(self, var_names, priority_level):
        ...
        # remove any high priority vars from medium priority group
        self.medium = self.medium.difference(self.high)

        self.low = self.low.difference(self.core)
        # remove any high priority vars from low priority group
        self.low = self.low.difference(self.high)
        # remove any medium priority vars from low priority group
        self.low = self.low.difference(self.medium)
    ...
    def consistency_check(self):
        ...
        for this_p in PRIORITY_LEVELS:
            ...
            for p in other_p:
                assert getattr(self, this_p).intersection(
                    getattr(self, p)) == set()
    ...
    def __repr__(self):
        ...
        for p in PRIORITY_LEVELS:
            ...
            if break_up_compound_name and UNIQUE_VAR_NAME == 'compound name':
                ...
                for table in tables:
                    varnames = sorted(
                        set([t[1] for t in lt if t[0] == table]), key=sortby)
                    n = len(varnames)
